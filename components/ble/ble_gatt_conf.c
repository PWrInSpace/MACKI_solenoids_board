...
esp_err_t ble_gatt_init(ble_gatts_t *gatts_conf) {
    ...
    if (gatts_conf->profiles_num == 0 || gatts_conf->profiles_num > GATTS_PROFILES_NUM_MAX) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(BLE_GATT_TAG, "Failed to register GATT callback: %s", esp_err_to_name(ret));
        return ret;
    }
