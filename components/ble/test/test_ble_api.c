...
typedef enum { PROFILE_A, PROFILE_B, PROFILE_TOTAL_NUM } ble_gatt_profile_names_t;
...
void gatt_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
                                  esp_ble_gatts_cb_param_t *param);
...
const esp_gatts_attr_db_t gatt_profile_b_db[HRS_IDX_NB] = {
    // Service Declaration
    [IDX_SVC] = {{ESP_GATT_AUTO_RSP},
                 {ESP_UUID_LEN_16, (uint8_t *)&secondary_service_uuid, ESP_GATT_PERM_READ,
                  sizeof(uint16_t), sizeof(GATTS_SERVICE_UUID_TEST2),
...
ble_gatts_t ble_gatt_conf = {
    ...
    .profiles = {
        [PROFILE_A] = {.gatts_cb = gatt_profile_a_event_handler, .gatts_if = ESP_GATT_IF_NONE},
        [PROFILE_B] = {.gatts_cb = gatt_profile_b_event_handler, .gatts_if = ESP_GATT_IF_NONE}}};
...
void gatt_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT: {
            ...
            if (set_dev_name_ret) {
                ESP_LOGE(BLE_TEST_TAG, "set device name failed, error code = %x", set_dev_name_ret);
            }
#ifdef CONFIG_SET_RAW_ADV_DATA
            esp_err_t raw_adv_ret =
                esp_ble_gap_config_adv_data_raw(raw_adv_data, sizeof(raw_adv_data));
            if (raw_adv_ret) {
                ESP_LOGE(BLE_TEST_TAG, "config raw adv data failed, error code = %x ", raw_adv_ret);
            }
            adv_config_done |= ADV_CONFIG_FLAG;
            esp_err_t raw_scan_ret =
                esp_ble_gap_config_scan_rsp_data_raw(raw_scan_rsp_data, sizeof(raw_scan_rsp_data));
            if (raw_scan_ret) {
                ESP_LOGE(BLE_TEST_TAG, "config raw scan rsp data failed, error code = %x",
                         raw_scan_ret);
            }
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
#else
            ...
            if (ret) {
                ESP_LOGE(BLE_TEST_TAG, "config scan response data failed, error code = %x", ret);
            }
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
#endif
            esp_err_t create_attr_ret =
                esp_ble_gatts_create_attr_tab(gatt_profile_a_db, gatts_if, HRS_IDX_NB, SVC_INST_ID);
            if (create_attr_ret) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep) {
                ...
                // GATTS_DEMO_CHAR_VAL_LEN_MAX.
                ESP_LOGI(BLE_TEST_TAG, "GATT_WRITE_EVT, handle = %d, value len = %d, value :",
        ...
        case ESP_GATTS_CONNECT_EVT:
            ESP_LOGI(BLE_TEST_TAG, "ESP_GATTS_CONNECT_EVT, conn_id = %d", param->connect.conn_id);
...
void gatt_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT: {
            esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(gatt_profile_b_db, gatts_if,
                                                                      HRS_IDX_NB, SVC_INST_ID2);
            if (create_attr_ret) {
                ESP_LOGE(BLE_TEST_TAG, "create attr table failed, error code = %x",
                         create_attr_ret);
            }
...
void example_prepare_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env,
                                     esp_ble_gatts_cb_param_t *param) {
    ESP_LOGI(BLE_TEST_TAG, "prepare write, handle = %d, value len = %d", param->write.handle,
             param->write.len);
...
TEST_CASE("Ble init test", "[BLE_API]") { TEST_ASSERT_EQUAL(BLE_OK, ble_init(&ble_conf)); }
